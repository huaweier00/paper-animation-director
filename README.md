...
| 镜头声称表现动作，画面却没做 | 对镜头负责呈现的关键动作提供静音可读证据；允许有依据的画外、省略和结果式叙事 |
...
7. **基准合同**：先选一段足以暴露本项目真实风险的代表性段落，证明身份、接触、物理、方向、因果、声音和字幕；时长由风险与镜头设计决定。
...
再为代表性高风险基准段落建立空间合同；不得提前批量生成其他镜头素材。
```
...
基准段落应覆盖项目最难的导演或制作问题：共同负重、扶起、拦驾、命令响应、灯—幕—影光学关系、火水传播、完整工艺步骤、细微表演或有意静止。用足以暴露风险的时长证明身份、头脸完整性、接触、比例、方向、因果、声音和字幕。
...
需要让“真实行走”在当前镜头中成立时，使用足以表达承重、换脚和前进关系的完整身体状态。下面是常用的六状态示例，不是所有画风和镜头都必须照搬的帧数：
...
当镜头负责表现命令被执行时，需要让状态转变可见。例如“放下武器”可以呈现：
